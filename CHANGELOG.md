...
### Added
- Initial release of `ebook_pp.py`, a SABnzbd post-processing script for eBooks. After a download finishes it files the eBook(s) into `/media/Storage/eBooks/{Author Name}/`, deletes the leftover completed job folder, and removes the job's entry from SABnzbd via the History API. Pure Python standard library — no external binaries or packages — so it runs as-is inside the stock `lscr.io/linuxserver/sabnzbd` image.
